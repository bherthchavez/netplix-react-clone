...
const Movie = ({item}) => {
    ...
    const saveShow = async ()=>{
        if(user?.email){
            ...
            alert('Plase login to save a movie.');
        }
    }
    ...
    return (
        <div className='w-[160px] sm:w-[200px] md:w-[240px] lg:[280px] inline-block cursor-pointer relative p-2'>
            ...
            <div className='absolute top-0 left-0 w-full h-full hover:bg-blac/80k opacity-0 hover:opacity-100 text-white'>
                ...
                <p className='white-space-normal text-xm md:text-sm font-bold flex justify-center items-center h-full text-center'>{item?.title}</p>
                <p onClick={saveShow}>
